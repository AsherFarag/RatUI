...
namespace RatUI
{
    namespace
    {
        Vec2<Unit> ResolveChildArrangeSize( const LayoutNode& a_Child, Vec2<Unit> a_InnerSize )
        {
    ...
    Vec2<Unit> MeasureLayoutNode( LayoutNode& a_Node, Vec2<Unit> a_AvailableSize )
    {
...

        {
            ...
            if ( numFlow > 0 )
            {
                ...
                if ( s.LayoutType == ELayoutType::Vertical )
                    contentSize[1] -= s.Spacing;
            }

            contentSize = contentSize + padding;

            if ( s.WidthMode == ESizingMode::Content )
                desired[0] = contentSize[0];
            if ( s.HeightMode == ESizingMode::Content )
                desired[1] = contentSize[1];
        }
    ...
    {
        return ( a_Child.Style.SelfAlign != EAlignment::Inherit ) ? a_Child.Style.SelfAlign : a_Parent.Style.ChildAlign;
    }
    ...
    void ArrangeOverlay( LayoutNode& a_Node, Rect<Unit> a_Inner )
    {
        a_Node.ForEachChild( [&]( LayoutNode& child )
            ...
            if ( child.Style.WidthMode == ESizingMode::Flex )
                childSize[0] = a_Inner.Size[0];
            if ( child.Style.HeightMode == ESizingMode::Flex )
    ...
    Unit AlignCrossAxis( Unit a_ChildSize, Unit a_ParentPos, Unit a_ParentSize, EAlignment a_Align, bool a_IsMainAxisHorizontal )
    {
        bool center = a_IsMainAxisHorizontal ? ( a_Align & EAlignment::VCenter ) : ( a_Align & EAlignment::HCenter );
        bool end = a_IsMainAxisHorizontal ? ( a_Align & EAlignment::Bottom ) : ( a_Align & EAlignment::Right );

        if ( center )
            return a_ParentPos + ( a_ParentSize - a_ChildSize ) * 0.5f;
        if ( end )
            return a_ParentPos + a_ParentSize - a_ChildSize;
        return a_ParentPos;
    }
    ...
    void ArrangeLinear( LayoutNode& a_Node, Rect<Unit> a_Inner )
    {
        ...
        a_Node.ForEachChild( [&]( const LayoutNode& child )
            ...
            if ( child.Style.PositionMode == EPositioningMode::Anchored )
                return;
            if ( !Visibility::AffectsLayout( childVisibility ) )
                return;
        ...
        a_Node.ForEachChild( [&]( LayoutNode& child )
            ...
            if ( isHz && ( align & EAlignment::VStretch ) == EAlignment::VStretch )
                childSize[1] = a_Inner.Size[1];
            if ( !isHz && ( align & EAlignment::HStretch ) == EAlignment::HStretch )
                childSize[0] = a_Inner.Size[0];
            ...
            const Unit advance = isHz ? childSize[0] + child.Style.Margin.Horizontal() : childSize[1] + child.Style.Margin.Vertical();
