...
namespace RatUI
{
    ...
    template<>
    struct std::formatter<EAlignment>
    {
        ...
        auto format(EAlignment value, FormatContext& ctx) const
        {
            ...
            auto append = [&](StringView s, bool& first)
            {
                if (!first)
                {
                    *out++ = ' ';
    ...
    template<>
    struct std::formatter<ELayoutDirection>
    {
        constexpr auto parse(std::format_parse_context& ctx)
        {
            return ctx.begin(); // no custom format options
        ...
        template<typename FormatContext>
        auto format(ELayoutDirection value, FormatContext& ctx) const
        {
    ...
    template<>
    struct std::formatter<Constraints>
    {
        constexpr auto parse(std::format_parse_context& ctx)
        {
            return ctx.begin(); // no custom format options
        ...
        template<typename FormatContext>
        auto format(const Constraints& value, FormatContext& ctx) const
        {
