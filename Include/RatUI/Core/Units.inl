...
namespace RatUI
{
    ...
    struct Radians
    {
        ...
        constexpr Radians( Degrees<T> a_Degrees );
    ...
    struct Degrees
    {
        ...
        constexpr Degrees( Radians<T> a_Radians ) : Value( RadToDeg( a_Radians.Value ) ) {}

        // === Conversion Operators ===

        constexpr operator Radians<T>() const { return Radians<T>{ DegToRad( Value ) }; }
