...
namespace RatUI
{
    ...
    public:
         ...
         * @param a_Texture The texture to update. Must be a valid texture that was previously created and not yet destroyed.
         ...
         */
        virtual bool UpdateTexture( const TextureHandle& a_Texture, u32 a_MipLevel, Rectu a_Region, const void* a_Data, size a_DataSizeBytes ) = 0;

        /**
         * @brief Destroys a texture resource, freeing its associated GPU memory. After this call, the texture is no longer valid and should not be used in draw calls.
         * @param a_Texture The texture to destroy. Must be a valid texture that was previously created and not yet destroyed.
         */
        virtual void DestroyTexture( const TextureHandle& a_Texture ) = 0;

        /**
         * @brief Checks if a given TextureID corresponds to a valid texture resource that is currently managed by the renderer.
         * @param a_Texture The texture to validate.
         * @return true if the texture is valid and corresponds to an existing texture resource, false otherwise.
         */
        virtual bool IsValidTexture( const TextureHandle& a_Texture ) const = 0;

        /**
         * @brief Retrieves information about a texture resource, such as its size, format, and sampler settings.
         * @param a_Texture The texture to query. Must be a valid texture that was previously created and not yet destroyed.
         * @return An optional TextureInfo structure containing the texture's information if the texture is valid, or NullOpt if the texture is invalid.
         ...
         */
        virtual Optional<TextureInfo> QueryTextureInfo( const TextureHandle& a_Texture ) const = 0;
    };
