...
void OnRender()
{
  ...
  // ---------------- ANIMATION ----------------
  f32 time = static_cast<f32>( GetTime() );
  ...
  // ---------------- LAYOUT PASS ----------------
  i32 windowWidth = GetScreenWidth();
  i32 windowHeight = GetScreenHeight();
  ...
  {
      ::Color rectCol = {r, g, b, 255};
      Rectangle rect{
          (f32)node.Layout.FinalRect.Origin[0],
          (f32)node.Layout.FinalRect.Origin[1],
...
int main() {
  ...
  InitWindow(1280, 720, "RatUI Raylib Demo");
  SetTargetFPS(60);
